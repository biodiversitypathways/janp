...
run_trend <- function(spp, ecor){
  ...
  if(ecor == "Alpine"){
      ...
      janp_trend <- janp_ready_trend |>
        ...
        dplyr::select(organization, project_id, longitude, latitude, location, location_id, task_id, ecoregion, landcover, prop_cover, VEGETATION_DENSITY, recording_date_time, observer, species_code, individual_order, abundance, detection_time) |>
        distinct() |>
      ...
      fit <- svabu(individual_order ~ year + landcover + prop_cover + VEGETATION_DENSITY | julian + hour + observer, data = jpt)
      ...
      return(trend_result_alpine <- tibble(
        species_code = spp,
        ecoregion = "Alpine",
        tau          = tau,
        p_value      = p_value,
  ...
  mutate(year = year(recording_date_time),
         hour = hour(recording_date_time),
         julian = yday(recording_date_time)) |>
  dplyr::select(organization, project_id, longitude, latitude, location, location_id, task_id, ecoregion, landcover, prop_cover, recording_date_time, observer, species_code, individual_order, abundance, detection_time) |>
  distinct() |>
  wt_tidy_species(remove = c("mammal", "amphibian", "abiotic", "insect", "human", "unknown"), zerofill = T) 

  # 2. Combine with zero sites
  base_zero <- janp_trend |>
    dplyr::select(location, ecoregion, landcover, prop_cover, recording_date_time, observer) |>
    distinct()

  spp_max <- janp_trend |>
    filter(species_code == "VATH") |>
    group_by(location, ecoregion, landcover, prop_cover, recording_date_time, observer) |>
    summarise(individual_order = max(individual_order, na.rm = TRUE)) |>
    ungroup()
  ...
  jpt <- base_zero |>
    left_join(spp_max,
              by = c("location", "ecoregion", "landcover", "prop_cover", "recording_date_time", "observer")) |>
    mutate(individual_order = tidyr::replace_na(individual_order, 0)) |>
    mutate(year = year(recording_date_time),
           hour = hour(recording_date_time),
           julian = yday(recording_date_time)) |>
  ...
  # 3. Fit svabu: year only in abundance component
  fit <- svabu(individual_order ~ year + landcover | julian + hour + observer, data = jpt)
  ...
  return(trend_result_forested <- tibble(
    species_code = spp,
    ecoregion = "Forested",
    tau          = tau,
    p_value      = p_value,
