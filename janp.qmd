...
ggplot(yearly_rao, aes(x = year, y = meanRao)) +
  ...
  labs(
    ...
    subtitle = "Mann-Kendall Test, Linear Model, Mixed Effects, and Segmented Trends",
    x = "Year",
...
This section presents the normalized species abundance trends, with plots illustrating changes over time across dietary, migratory, and habitat guilds (@fig-normalized-count-mdm-year) and annual counts broken down by ecoregions such as Alpine and Montane (@fig-annual-count), complemented by a table summarizing linear model outputs for species trends (@tbl-normalized-count-mdm-year).
...
tot_a_filtered <- janp_main |>
  ...
  mutate(abundance = max(individual_order)) |>
  ungroup() |>
...
kable(trend_models |> arrange(species_code) |> dplyr::select(-c(best_model_species_code, best_model_summary, best_model_plot)), caption = "Trend Models Summary", format = "html")
...
kable(bind_rows(trend_models_trait, trend_models_habitat, trend_models_dietary) |> dplyr::select(guild, model_name, AIC), caption = "Trend Models Summary", format = "html")
