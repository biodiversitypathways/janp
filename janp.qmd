...
coefz_long |>
  drop_na(term) |>
  mutate(alpha_sig = factor(case_when(p.value < 0.05 ~ 1, is.na(p.value) ~ 0.7, TRUE ~ 0.8))) |>
  ggplot(aes(x = term, y = estimate, shape = ecoregion, colour = species_code, alpha = alpha_sig)) +
