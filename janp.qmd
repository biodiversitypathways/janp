...
fires_in_janp <- fires_in_janp |> filter(!grepl('Chetamon',ALIAS)) |> filter(!YEAR == 2024)
...
ggplot(dist_df, aes(x=group, y=dist_to_centroid)) +
  geom_boxplot() +
...
Temporal trends in abundance were estimated using single-visit abundance models implemented in the `detect::svabu()` function (@solymos2012conditional). This framework jointly models site-level abundance and detection probability from single-visit counts, providing unbiased estimates without requiring repeated surveys. In the models, year was included as a covariate in the abundance component to capture long-term trends, while julian date, hour of day and observer were included in the detection component to account for within-season and diurnal variation in detectability.  The resulting detection-corrected expected abundance per site-visit (λ) was then aggregated by year to produce mean annual abundance indices. These indices were subsequently used to evaluate temporal trends in species abundance and community composition.

#### Trend estimation

Temporal trends were quantified using the Mann–Kendall test (@mann1945non; @hamed2009), which detects monotonic directional change, and Sen’s Slope (@Sen01121968) to estimate the magnitude of those trends. Both tests were implemented via the `modifiedmk` package (@hamed1998) and applied to raw count data as well as detection-corrected abundance estimates (λ̂). Comparing these two datasets allowed us to assess how accounting for detection influenced the estimated direction and rate of change. This dual approach provided both a robust, distribution-free evaluation of trend significance and a quantitative measure of annual rate of change.
...
if(nrow(spp_max) < 20) { message(paste0("Probably an error in Alpine with ", spp))}
...
fit <- svabu(individual_order ~ year | julian + hour + observer, data = jpt)
...
return(trend_result_rcki_alpine <- tibble(
  species_code = spp,
  ecoregion = "Alpine",
  tau          = tau,
  p_value      = p_value,
...
return(trend_result_rcki_forested <- tibble(
  species_code = spp,
  ecoregion = "Forested",
  tau          = tau,
  p_value      = p_value,
...
trendz <- bind_rows(
  run_trend("VATH", "Alpine"), run_trend("VATH", "Forested"),
  run_trend("DEJU", "Alpine"), run_trend("DEJU", "Forested"),
  run_trend("HETH", "Alpine"), run_trend("HETH", "Forested"),
  run_trend("FOSP", "Alpine"), run_trend("FOSP", "Forested"),
  run_trend("CHSP", "Alpine"), run_trend("CHSP", "Forested"),
  run_trend("YRWA", "Alpine"), run_trend("YRWA", "Forested"),
  run_trend("SWTH", "Alpine"), run_trend("SWTH", "Forested"),
  run_trend("AMRO", "Alpine"), run_trend("AMRO", "Forested"),
  run_trend("PISI", "Alpine"), run_trend("PISI", "Forested"),
  run_trend("RCKI", "Alpine"), run_trend("RCKI", "Forested"),
  run_trend("GCKI", "Alpine"),   run_trend("GCKI", "Forested"),
  run_trend("WCSP", "Alpine"),   run_trend("WCSP", "Forested"),
  run_trend("GCSP", "Alpine"),   run_trend("GCSP", "Forested"))

  run_trend("WTSP", "Alpine"),   run_trend("WTSP", "Forested"),
  run_trend("RBNU", "Alpine"),   run_trend("RBNU", "Forested"),
  run_trend("WAVI", "Alpine"),   run_trend("WAVI", "Forested"),
  run_trend("LISP", "Alpine"),   run_trend("LISP", "Forested"),
  run_trend("OCWA", "Alpine"),   run_trend("OCWA", "Forested"),
  run_trend("BRSP", "Alpine"),   run_trend("BRSP", "Forested"),
  run_trend("AMPI", "Alpine"),   run_trend("AMPI", "Forested"))


  #run_trend("MOCH", "Alpine"),   run_trend("MOCH", "Forested"),
  run_trend("CORA", "Alpine"),   run_trend("CORA", "Forested"),
  run_trend("WIWA", "Alpine"),   run_trend("WIWA", "Forested"),
  run_trend("TEWA", "Alpine"),   run_trend("TEWA", "Forested"),
  run_trend("COYE", "Alpine"),   run_trend("COYE", "Forested"),
  run_trend("HAFL", "Alpine"),   run_trend("HAFL", "Forested")
)
