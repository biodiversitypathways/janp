...
obs_wide <- obs_dups %>%
  pivot_longer(
    cols = `Maya Meron`:`Kelsey Bell`,
    names_to = "observer",
    values_to = "count"
  ) %>%
  pivot_wider(names_from = observer, values_from = count, values_fill = 0)

# Observer names
observer_cols <- colnames(obs_wide)[!(colnames(obs_wide) %in% c("location", "recording_date_time", "recording_id", "species_code"))]

# Initialize empty tibble for pairwise correlations
...
for(i in 1:(length(observer_cols)-1)) {
  for(j in (i+1):length(observer_cols)) {
    o1 <- observer_cols[i]
    o2 <- observer_cols[j]
    c <- cor(obs_wide[[o1]], obs_wide[[o2]], use = "pairwise.complete.obs")
