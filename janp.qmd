...
for (sp in mdm) {
  for (ec in unique(tot_abundance$ecoregion)) {
    ...
    fit <- svabu(individual_order ~ year | 1, data = simp)
...
Modified Mann–Kendall trend analyses revealed distinct patterns in detection‐corrected abundance across ecoregions. Significant positive trends (τ ≈ 1, p < 0.001) were found for most species in the Montane and Alpine regions, indicating consistent increases over time. Species showing the strongest upward trends included Dark-eyed Junco (DEJU), Fox Sparrow (FOSP), Swainson’s Thrush (SWTH), White-crowned Sparrow (WCSP), and Red-breasted Nuthatch (RBNU), with Sen’s slopes ranging from 0.007 to 0.03. In contrast, several species exhibited significant declines in Montane sites—such as American Robin (AMRO), Chipping Sparrow (CHSP), Ruby-crowned Kinglet (RCKI), Orange-crowned Warbler (OCWA), and Yellow-rumped Warbler (YRWA)—suggesting region-specific decreases in abundance. Interestingly, some species (e.g., CHSP, HETH, YRWA) showed opposite trends between Alpine and Montane regions, implying possible elevational or habitat-related shifts. Using Mann–Kendall tests and Sen’s slope estimates, only a few species–ecoregion combinations exhibited notable trends exceeding ±2.5% per year. Specifically, AMPI (Alpine), HETH (Alpine), GCSP (Montane), and RBNU (Montane) showed increases greater than 2.5% annually, while all other species were categorized as stable under this threshold. The majority of species exhibited low-magnitude changes, indicating relative stability in abundance over the study period despite statistically significant monotonic trends in some cases. These results highlight that, while some species may be increasing locally, broad-scale temporal shifts in abundance are limited, suggesting overall community composition remains largely consistent across both Montane and Alpine ecoregions.
...
ggplot(lambda_all_flagged, aes(x = year, y = lambda_hat, color = trend_flag)) +
  ...
  geom_line(size = 1.1) +
  facet_grid(ecoregion ~ species_code, scales = "free_y") +
