...
To analyze trends in bird abundance in Jasper National Park from 2007 to 2024, we utilized point count data to evaluate changes at the species and guild levels. Abundance was defined as the count of individuals detected during point counts, rather than as a density x area relationship. Observations were first filtered to include only data from the study period and were classified by species and functional guilds using established ecological trait databases (`tbl-guilds`). Spatial clustering was addressed by grouping data by individual locations and transects. Georeferenced transect locations were used to define these strata. A time-to-first-detection (TTFD) variable was included, when available, to control for potential biases in detectability.
