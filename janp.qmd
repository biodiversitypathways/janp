...
To evaluate community-level ecological responses, we examined temporal changes in functional diversity, species richness, and community evenness. Functional diversity was quantified using Rao’s Q (Rao, 1982; Laliberté & Legendre, 2010) via the dbFD() function in the FD package (Laliberté, Legendre, & Shipley, 2014). We also calculated species richness (number of unique species per location per year) and Shannon’s diversity index, which integrates richness and evenness to describe community structure. These metrics were modeled through time using linear, mixed-effects, and segmented regression models to detect both gradual and threshold-type changes. Results were summarized graphically by ecoregion and functional guild to highlight spatial variation in diversity trajectories.

### Trend analysis

To quantify temporal changes in bird populations and community composition from 2007 to 2024, we analyzed trends in species-specific abundance, forest and alpine assemblages, and functional guilds. Analyses were designed to separate biological change from potential sampling and methodological effects, ensuring that observed patterns represented genuine ecological responses. This was achieved through a multi-step framework that (1) modeled detection probability and methodological variability, (2) estimated detection-corrected abundance, and (3) evaluated long-term directional trends and associated shifts in functional and community-level diversity.

#### Detectability assessment and methodological effects

Because transcription methods varied over time (Table 2), potentially influencing both species detectability and data consistency, we applied generalized linear mixed models (GLMMs) using the lme4 package (Bates et al., 2015). The GLMMs quantified the effects of year and survey method on time-to-first detection, allowing us to test whether detectability changed systematically across years or protocols. Including location as a random effect (1 | location) accounted for repeated observations at the same sites and controlled for site-level variation in baseline detection rates. This approach provided a flexible framework for distinguishing true temporal change from artifacts of survey design or observer methodology.

#### Detection-corrected abundance estimation

Results from the GLMMs were then used to diagnose and adjust for potential detection bias in subsequent abundance analyses. Specifically, evidence of year- or method-dependent differences in detectability guided the inclusion of these covariates in single-visit abundance (svabu) models implemented in the detect package (Sólymos, Lele, & Bayne, 2012). 
The svabu framework jointly models abundance and detection probability within a single-visit design, providing unbiased abundance estimates without requiring repeated surveys or closed-population assumptions. In these models, year was included as a covariate in the abundance component to capture long-term temporal trends, while Julian date and time of day were included in the detection component to account for within-season and diurnal variation in detectability (e.g., svabu(individual ~ year | year + julian + hour)). 
Although the QPAD (Quantitative Population Assessment of Detectability) framework (Sólymos et al., 2013; Sólymos & Matsuoka, 2016) can also estimate abundance and density while explicitly modeling availability, perceptibility, and effective area, it requires detailed metadata on distance or duration and often relies on calibration across survey protocols. Because this dataset consisted primarily of single-visit point counts without consistent distance or duration fields, the svabu approach was considered more appropriate for this study. svabu provided a statistically robust, computationally efficient alternative that could incorporate detection covariates directly from the existing dataset while maintaining comparability across years and sites. This approach produced year-specific mean abundance estimates (λ̂) corrected for imperfect detection. These λ̂ values were then used to evaluate temporal trends in abundance and community composition.

#### Trend estimation

Temporal trends were quantified using the Mann–Kendall test (Mann, 1945; Hamed & Rao, 1998), which detects monotonic directional change, and Sen’s Slope (Sen, 1968) to estimate the magnitude of those trends. Both tests were implemented via the modifiedmk package (Dawood, 2017) and applied to raw count data as well as detection-corrected abundance estimates (λ̂). Comparing these two datasets allowed us to assess how accounting for detection influenced the estimated direction and rate of change. This dual approach provided both a robust, distribution-free evaluation of trend significance and a quantitative measure of annual rate of change.
...
shannon_d <- janp_main |> 
  ...
  dplyr::select(location, recording_date_time, species_code, species_common_name, individual_order, abundance) |>
  distinct() |>
...
janp_trend <- janp_main |>
  filter(ecoregion %in% c("Alpine")) |>
  mutate(year = year(recording_date_time),
         hour = hour(recording_date_time),
         julian = yday(recording_date_time)) |>
  dplyr::select(organization, project_id, longitude, latitude, location, location_id, task_id, ecoregion, recording_date_time, observer, species_code, individual_order, abundance, detection_time) |>
  distinct() |>
...
base_zero <- janp_trend |>
  dplyr::select(location, ecoregion, recording_date_time, observer) |>
  distinct()

yrwa <- janp_trend |>
    filter(species_code == "RCKI") |>
  group_by(location, ecoregion, recording_date_time, observer) |>
  summarise(individual_order = max(individual_order, na.rm = TRUE)) |>
  ungroup()

jpt <- base_zero |>
  left_join(yrwa,
            by = c("location", "ecoregion", "recording_date_time", "observer")) |>
...
# 3. Fit svabu: year only in abundance component
fit <- svabu(individual_order ~ year | julian + hour, data = jpt)

# 4. Standardize detection
ref_vals <- jpt %>%
  summarise(
...
jpt$lambda_hat <- predict(fit, newdata = jpt %>% mutate(julian = ref_vals$julian, hour = ref_vals$hour), type = "response")

# 5. Aggregate to annual index
lambda_year <- jpt %>%
  group_by(year) %>%
  summarise(lambda_hat = mean(lambda_hat, na.rm = TRUE), .groups = "drop") %>%
  arrange(year)
...
trend_class <- dplyr::case_when(
  pct_change >  2.5  ~ "Increasing",
  pct_change < -2.5  ~ "Decreasing",
  TRUE               ~ "Stable"
)

trend_result_rcki_alpine <- tibble(
  species_code = "RCKI",
  ecoregion = "Alpine",
  tau          = tau,
  p_value      = p_value,
...
# 1. Summarize per site-visit using max()
janp_trend <- janp_main |>
  filter(ecoregion %in% c("Montane", "Upper Subalpine", "Lower Subalpine")) |>
  mutate(year = year(recording_date_time),
         hour = hour(recording_date_time),
         julian = yday(recording_date_time)) |>
  dplyr::select(organization, project_id, longitude, latitude, location, location_id, task_id, ecoregion, recording_date_time, observer, species_code, individual_order, abundance, detection_time) |>
...
trend_result_rcki_forested <- tibble(
  species_code = "RCKI",
  ecoregion = "Forested",
  tau          = tau,
  p_value      = p_value,
...
trendz <- bind_rows(
  trend_result_vath_alpine, trend_result_vath_forested,
  trend_result_deju_alpine, trend_result_deju_forested,
  trend_result_heth_alpine, trend_result_heth_forested,
  trend_result_fosp_alpine, trend_result_fosp_forested,
  trend_result_chsp_alpine, trend_result_chsp_forested,
  trend_result_yrwa_alpine, trend_result_yrwa_forested,
  trend_result_swth_alpine, trend_result_swth_forested,
  trend_result_amro_alpine, trend_result_amro_forested,
  trend_result_pisi_alpine, trend_result_pisi_forested,
  trend_result_rcki_alpine, trend_result_rcki_forested
)


# 7. Plot results
ggplot(lambda_year, aes(x = year, y = lambda_hat)) +
  geom_point(size = 3) +
  geom_line(group = 1, color = "steelblue", size = 1) +
  labs(
    x = "Year",
    y = "Detection-corrected abundance trend",
    title = "Annual abundance index trend"
  ) +
...
#trend_h <- bind_rows(trend_result, trend_g)
