...
datatable(guilds |> left_join(guild_activity |> dplyr::select(species_code, n)), 
          options = list(
            searching = TRUE,  
            paging = TRUE,    
            pageLength = 10   
...
#| tbl-cap: Summary of trend percent change by species and ecoregion. Colours indicated departure from baseline to increasing (+2.5%) or decreasing (-2.5%) territory.
#| fig-width: 8
#| fig-height: 12

ggplot(trend_table, aes(x = reorder(species_common_name, -trend_pct_change), y = trend_pct_change, colour = trend_jasper)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  facet_wrap(~trend_ecoregion) +
  coord_flip() + 
  theme_bw() +
  xlab("Species") +
