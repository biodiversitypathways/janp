...
Before adopting WildTrax, analysts excluded the initial 20 seconds to 1.5 minutes of recordings to reduce human impact on detection probability, then logged the first detection time per species. Recordings are now uploaded as clean 10-minute files with the initial noise removed. Transcription methods varied over time @tbl-transcriptions, affecting data consistency. To ensure comparability, abundance analyses are restricted to the first 3.33 minutes, the only consistently applied method across years. A cap of three individuals per species was applied in 2021–22 but is unlikely to impact results given the short timeframe. If feasible, time-to-first detection can be included to account for detection probability differences. We also verified tags @tbl-verified and @tbl-tmtt for abundance balancing.
