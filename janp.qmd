...
janp_main <- bind_rows(old_s_max, janp_aru) |>
  ...
  # rowwise() |>
  # mutate(temperature = case_when(is.na(temperature) ~ Temperature, TRUE ~ temperature),
  #        sky = case_when(is.na(sky) ~ Sky, TRUE ~ Sky),
  #        wind = case_when(is.na(wind) ~ Wind, TRUE ~ Wind)) |>
  # ungroup() |>
  # group_by(location, recording_date_time) |>
  # fill(temperature, sky, wind, .direction = "down") |>
  # ungroup()
```
...
datatable(guilds |> left_join(guild_activity |> dplyr::select(species_code, n) |> relocate(n, .after = species_code)), 
          options = list(
            searching = TRUE,  
            paging = TRUE,    
            pageLength = 10   
...
\logit(pi)=α0+α1 temperaturei+α2 windi+α3 juliani+α4 houri.\logit(p_i) = \alpha_0 + \alpha_1\,\text{temperature}_i + \alpha_2\,\text{wind}_i + \alpha_3\,\text{julian}_i + \alpha_4\,\text{hour}_i .\logit(pi​)=α0​+α1​temperaturei​+α2​windi​+α3​juliani​+α4​houri​.
...
run_trend <- function(spp) {
  ...
  if (nrow(spp_max |> filter(species_code == spp)) < 20) {
    ...
    jpt <- base_zero |> 
      ...
      mutate(canopy_species = factor(canopy_species), 
             lc_group = case_when(landcover %in% c("Tree Coniferous", "Shrub Low", "Herbaceous Mixed", "Bedrock") ~ landcover, TRUE ~ "Other"), .after = landcover) |> 
      ...
      mutate(across(c(shrub, conf, herb, other, bedrock, canopy_cover, avg_height, julian, hour, temperature, wind), ~ as.numeric(scale(.x)))) |> 
      mutate(year_c = scale(year)) |>
      distinct() |>
      mutate(ecoregion = case_when(location == "WILCOX-5" ~ "Alpine", TRUE ~ ecoregion))
    
    fit <- svabu(individual_order ~ year_c + shrub + conf + avg_height + canopy_cover | temperature + wind + julian + hour, data = jpt, zeroinfl = F)
    model_fit_alpine <- if (!is.null(fit)) {summary(fit)} else {NA}
    ...
    jpt$lambda_hat <- NA
    jpt$lambda_hat[!is.na(newdata$year_c) & !is.na(newdata$avg_height)] <- 
  predict(fit, newdata = newdata, type = "response")
    ...
    if (nrow(spp_max |> filter(species_code == spp)) < 20) {
      ...
      fit <- svabu(individual_order ~ year_c + shrub + conf + avg_height + canopy_cover | temperature + wind + julian + hour, data = jpt, zeroinfl = F)
      model_fit_montane <- if (!is.null(fit)) {summary(fit)} else {NA}
      ...
      newdata <- jpt %>%
          mutate(julian       = ref_zero_infl$julian,
                 hour         = ref_zero_infl$hour,
                 temperature = ref_zero_infl$temperature,
                 wind = ref_zero_infl$wind)
      
      jpt$lambda_hat <- predict(fit, newdata = newdata, type = "response")
      jpt$lambda_hat[!is.na(newdata$year_c) & !is.na(newdata$avg_height)] <- predict(fit, newdata = newdata, type = "response")
    ...
    return(list(bind_rows(trend_result_alpine, trend_result_montane), bind_rows(lambda_year_alpine, lambda_year_montane), bind_rows(lambda_year_alpine_plot, lambda_year_montane_plot), model_fit_alpine, model_fit_montane))
...
<!-- \[ -->
<!-- \text{individual\_order}_i \sim \text{Abundance}(\lambda_i), \quad -->
<!-- \log(\lambda_i) = \beta_0 + \beta_1\,\text{year}_{c,i} + \beta_2\,\text{shrub}_i + \beta_3\,\text{conf}_i + \beta_4\,\text{avg\_height}_i + \beta_5\,\text{canopy\_cover}_i, \quad -->
<!-- \text{Detection: } \logit(p_i) = \alpha_0 + \alpha_1\,\text{temperature}_i + \alpha_2\,\text{wind}_i + \alpha_3\,\text{julian}_i + \alpha_4\,\text{hour}_i -->
<!-- \] -->
...
coefz <- trendz |>
  ...
  select(species_code, n, model_alpine, model_montane)
...
plot_dat <- trendz |>
  ...
  unnest() |>
  dplyr::select(species_code, n, year, location, ecoregion, lambda_hat) |>
  distinct() |>
...
ggplot(plot_dat, aes(x = year, y = lambda_hat, color = ecoregion)) + 
  ...
  facet_wrap(~ forcats::fct_reorder(species_code, n, .desc = TRUE), scales = "free_y") +
  theme_bw() + 
  scale_colour_viridis_d(option = "cividis") + 
  ylim(0, 2) + 
...
ggplot(trend_table |> filter(!is.na(trend_jasper)),
