...
Human activities have been identified as key pressures and contributors to the global decline in forest wildlife (@allan2017recent). The repercussions of habitat fragmentation (@fahrig2003effects) and loss (@hanski2011habitat), climate change (@mantyka2012interactions, @sattar2021review, @abrahms2023climate), and increased access to sensitive areas exert direct and indirect pressures on forest biodiversity, particularly in managed regions in Canada (@lemieux2011state). Wildfire activity has significantly impacted montane bird monitoring in Jasper National Park. In 2023, two of the park’s three montane long-term monitoring transects have been burned, while the third is undergoing wildfire risk reduction efforts.
...
jj <- wt_location_distances(janp_main) |> filter(location_from %in% multiaru, distance < 300) |> inner_join(janp_main |> dplyr::select(location, ecoregion) |> distinct(), by = c("location_from" = "location"))
```
...
To analyze trends in bird abundance in Jasper National Park from 2007 to 2024, we utilized point count data to evaluate changes at the species and guild levels. Abundance was defined as the count of individuals detected during point counts, rather than as a density x area relationship. Observations were first filtered to include only data from the study period and were classified by species and functional guilds using established ecological trait databases (`tbl-guilds`). Spatial clustering was addressed by grouping data by individual locations and transects. Georeferenced transect locations were used to define these strata. A time-to-first-detection (TTFD) was examined to control for potential biases in detectability by comparing the mean TTFD of between each of the methods.

Community composition in Jasper National Park was analyzed using multivariate statistical approaches. Species-level observations were aggregated into a species-by-location matrix, with abundances recorded as the maximum individual order for each location and transect. Ecoregions were categorized into Alpine and Montane groups, with the latter further divided into Upper Subalpine, Lower Subalpine, and Montane zones. Redundancy analysis (RDA) was conducted using the vegan package in R to quantify the variation in community composition explained by ecoregions and visualize species-environment relationships with ordination plots. Differences in composition between Alpine and Montane groups were tested using permutational multivariate analysis of variance (PERMANOVA) with Bray-Curtis dissimilarity and 999 permutations under a reduced model.

Generalized linear mixed-effects models (GLMMs) were implemented in R (R Core Team, 2023) using the `lme4` package (Bates et al., 2015) to analyze abundance trends. Count data were modeled using a Poisson distribution or a negative binomial distribution in cases of overdispersion. Models included year, ecoregion, and their interaction as fixed effects, and location and transect as random effects to account for spatial clustering. Separate models were fitted for montane/subalpine and alpine ecoregions to facilitate region-specific analyses. Model selection was based on Akaike Information Criterion (AIC) to identify the best-fit models.

To evaluate trends, we used the Mann-Kendall test to assess the statistical significance of observed changes and Sen's Slope to estimate the magnitude of these trends over time. Functional diversity was assessed at the guild level using Rao’s Q, allowing us to explore changes in functional diversity across the study period. Breakpoint analysis was performed using the `segmented` package in R to identify significant shifts in abundance trends over time. Segmented models were fitted to detect structural changes that might warrant separating trend analysis into distinct phases. This ensured that important changes in abundance were not obscured by analyzing overall trends.

Ecoregional assessments were conducted by grouping abundance data for alpine and montane (upper subalpine, lower subalpine, and montane) ecoregions into singular datasets. Guild-level trends were similarly analyzed using Rao’s Q to detect changes in functional diversity.
...
guild_trend <- janp_main |>
  ...
  inner_join(locs_summary |> dplyr::select(Location, ecoregion), by = c("location" = "Location")) |>
...
guild_trend |>
  ggplot(aes(x = year, y = total_abundance, colour = guild_value)) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm") +
  facet_grid(ecoregion ~ guild_type) +
  scale_colour_viridis_d(option = "cividis", alpha = 0.7) + 
  theme_bw() +
  labs(x = "Year", y = "Total abundance of individuals per location")

guild_trend_clean <- guild_trend %>%
  group_by(ecoregion, year) %>%
  summarise(total_abundance = sum(total_abundance, na.rm = TRUE),
            sample_size = sum(sample_size, na.rm = TRUE),
            .groups = "drop")

# Pivot the data to make 'year' columns with abundance values
guild_trend_wide <- guild_trend_clean %>%
...
As noted in @fig-all-projects, there are many more historical and ongoing monitoring datasets that do fall outside the realm 
