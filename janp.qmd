...
- Report on trends in bird abundance by species and by guild in the montane (forest indicator) and alpine (alpine indicator) ecoregions, incorporating time-to-first-detection analysis
  - Summarize results by ecoregion as the percentage of species and guilds declining by ≥2.5% in both montane and alpine regions.
...
run_trend <- function(spp, summary = TRUE) {
    ...
    janp_trend <- janp_ready_trend |>
      ...
      dplyr::select(organization, project_id, longitude, latitude, location,
                    ...
                    VEGETATION_DENSITY,  temperature, sky, wind, canopy_species, canopy_cover, avg_height, recording_date_time, observer, species_code,
    ...
    base_zero <- janp_trend |>
      dplyr::select(location, ecoregion, landcover, prop_cover, VEGETATION_DENSITY, temperature, sky, wind, canopy_species, canopy_cover, avg_height, recording_date_time, observer) |>
      distinct()
    
    spp_max <- janp_trend |>
      filter(species_code == spp) |>
      group_by(location, ecoregion, landcover, prop_cover, VEGETATION_DENSITY, temperature, sky, wind, canopy_species, canopy_cover, avg_height, recording_date_time, observer, species_code) |>
    ...
    jpt <- base_zero |> 
      left_join(spp_max, by = c("location", "ecoregion", "landcover", "prop_cover", "VEGETATION_DENSITY", "temperature", "sky", "wind", "canopy_species", "canopy_cover", "avg_height", "recording_date_time", "observer")) |>
      ...
      dplyr::select(-c(landcover, VEGETATION_DENSITY)) |> group_by(location, ecoregion, canopy_species,  temperature, sky, wind, canopy_cover, avg_height, recording_date_time, observer, individual_order, year, hour, julian) |>
    ...
    fit <- svabu(individual_order ~ year + shrub + herb + conf + bedrock + canopy_cover + avg_height | julian + hour + observer + canopy_cover + avg_height + temperature + sky + wind, data = jpt)
...
lambda_year_alpine_plot <- jpt |> dplyr::select(year, location, ecoregion, lambda_hat) |> distinct() |> arrange(year)
    ...
    if (nrow(spp_max) < 20) {
      next(paste0("Probably an error in Montane with ", spp))
    }
    ...
    jpt <- base_zero |> 
      ...
      filter(!is.infinite(individual_order)) |> 
      mutate(canopy_species = factor(canopy_species), lc_group = case_when(landcover %in% c("Tree Coniferous", "Shrub Low", "Herbaceous Mixed", "Bedrock") ~ landcover, TRUE ~ "Other"), .after = landcover) |> 
      dplyr::select(-c(landcover, VEGETATION_DENSITY)) |> group_by(location, ecoregion, temperature, sky, wind, canopy_species, canopy_cover, avg_height, recording_date_time, observer, individual_order, year, hour, julian) |>
      pivot_wider(names_from = lc_group, values_from = prop_cover, values_fill = 0, values_fn = sum) |> 
      ungroup() |> 
    ...
    fit <- svabu(individual_order ~ year + shrub + herb + conf + canopy_cover + avg_height | julian + hour + observer + canopy_cover + avg_height + temperature + sky + wind, data = jpt)
...
lambda_year_montane_plot <- jpt |> dplyr::select(year, location, ecoregion, lambda_hat) |> distinct() |> arrange(year)
    ...
    return(list(bind_rows(trend_result_alpine, trend_result_montane), bind_rows(lambda_year_alpine, lambda_year_montane), bind_rows(lambda_year_alpine_plot, lambda_year_montane_plot), fit_summary_alpine, fit_summary_montane))
